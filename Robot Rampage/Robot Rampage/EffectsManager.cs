...
namespace Robot_Rampage
{
    static class EffectsManager
    {
        ...
        public static void AddSparksEffect(Vector2 location, Vector2 impactVelocity)
        {
            ...
            {
                Particle particle = new Particle(location - (impactVelocity / 60), Texture, ParticleFrame,
                    randomDirection((float)rand.Next(10, 20)), Vector2.Zero, 60, 20, Color.Yellow, Color.Orange);
        ...
        public static void AddExplosion(Vector2 location, Vector2 momentum, int minPointCount, int maxPointCount,
            int minPieceCount, int maxPieceCount, float pieceSpeedScale, int duration, Color initialColor, Color finalColor)
        {
            ...
            for (int x = 0; x < pieces; x++)
            {
                ...
                randomDirection(pieceSpeedScale) + momentum, Vector2.Zero, explosionMaxSpeed, duration, initialColor, finalColor));
            }
