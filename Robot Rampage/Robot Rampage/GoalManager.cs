...
namespace Robot_Rampage
{
    ...
    {
        #region Declarations
        private static List<ComputerTerminal> computerTerminals = new List<ComputerTerminal>();
        ...
        public static ComputerTerminal TerminalInSquare(Vector2 mapLocation)
        {
        ...
        public static void AddComputerTerminal()
        {
            ...
            if ((TerminalInSquare(tileLocation) != null) || (TileMap.IsWallTile(tileLocation)))
            {
                return;
            }

            if (Vector2.Distance(TileMap.GetSquareCenter(startX, startY), Player.BaseSprite.WorldCenter) < minDistanceFromPlayer)
            {
                return;
            }

            List<Vector2> path = PathFinder.FindPath(new Vector2(startX, startY), TileMap.GetSquareAtPixel(Player.BaseSprite.WorldCenter));
            ...
            if (path != null)
            {
                ...
                Sprite disabledSprite = new Sprite(new Vector2(squareRect.X, squareRect.Y), texture, initialDisabledFrame, Vector2.Zero);
                ComputerTerminal terminal = new ComputerTerminal(activeSprite, disabledSprite, new Vector2(startX, startY));
