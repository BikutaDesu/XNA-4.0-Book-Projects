    ...
    class Enemy
    {
        ...
        public Enemy(Texture2D texture, Vector2 location, Rectangle initialFrame, int frameCount)
        {
            ...
            for (int x = 1; x < frameCount; x++)
            {
                EnemySprite.AddFrame(
                    new Rectangle(initialFrame.X = (initialFrame.Width * x), initialFrame.Y,
        ...
        public void Update(GameTime gameTime)
        {
            if (IsActive())
            {
                ...
                EnemySprite.Rotation = (float)Math.Atan2(EnemySprite.Location.Y - previousLocation.Y, EnemySprite.Location.X - previousLocation.X);
                if (WaypointReached())
                {
                    if (waypoints.Count > 0)
                    {
